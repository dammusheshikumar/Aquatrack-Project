 ...
 * Services (loaded as a <script> tag in index.html). On success, calls
 * onCredential(idToken) with the raw Google ID token so the caller can
...
export default function GoogleSignInButton({ onCredential, onError }) {
  ...
  if (!GOOGLE_CLIENT_ID) {
    return <p className="text-xs" style={{ color: '#7A9097' }}>Google sign-in isn't configured (missing VITE_GOOGLE_CLIENT_ID).</p>
  }
